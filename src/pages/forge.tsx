...
export default function Forge() {
  ...
  const handleForge = async () => {
    if (!hasAll6 || alreadyForged || forging) return;
        ...
        >
          ...
          <div className="h-2 bg-[#04020c] rounded-full overflow-hidden border border-[#1a0a2e]">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${Math.min((unique / 6) * 100, 100)}%` }}
            ...
            >
              ...
              <div
                className={`w-full bg-[#0d0420] border-2 rounded-lg p-5 relative transition-all ${
                  hasAll6 ? "border-[#7c3aed]" : "border-[#1a0a2e]"
                }`}
                style={hasAll6 ? { boxShadow: "0 0 25px rgba(124,58,237,0.2)" } : {}}
              >
                <div className="absolute -top-1 -left-1 w-1.5 h-1.5 bg-[#22d3ee]" />
                <div className="absolute -top-1 -right-1 w-1.5 h-1.5 bg-[#22d3ee]" />
                ...
                <p
                  className={`font-['Press_Start_2P'] text-[10px] mb-1 ${
                    hasAll6 ? "text-[#c4b5d4]" : "text-[#2d1a4e]"
                  }`}
                >
                  GTD ARTIFACT
                </p>
                ...
                <motion.button
                  whileHover={hasAll6 && !forging ? { scale: 1.03 } : {}}
                  whileTap={hasAll6 && !forging ? { scale: 0.97 } : {}}
                  onClick={handleForge}
                  disabled={!hasAll6 || forging}
